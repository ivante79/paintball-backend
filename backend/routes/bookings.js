...
router.get("/all", authMiddleware, async (req, res) => {
  try {
    if (req.user.role !== "admin") {
      return res
        .status(403)
        .json({ message: "Potreban je administratorski pristup" });
    }
...
router.post("/", authMiddleware, async (req, res) => {
  const { bookingDate, timeSlot, numberOfPlayers, equipment, totalPrice } =
    req.body;
  ...
  try {
    ...
    if (existingBooking.rows.length > 0) {
      return res.status(400).json({ message: "Termin je već rezerviran" });
    }
    ...
    io.emit("new_booking", {
      message: "Nova rezervacija je kreirana",
      booking: booking,
    });
...
router.put("/:id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    io.to(`user_${req.user.id}`).emit("booking_updated", {
      message: "Rezervacija je ažurirana",
...
router.delete("/:id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    res.json({ message: "Rezervacija je uspješno otkazana" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Greška servera" });
  }
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      if (!req.file) {
        return res.status(400).json({ message: "Datoteka nije učitana" });
      }
      ...
      io.to(`user_${req.user.id}`).emit("receipt_uploaded", {
        message: "Potvrda o plaćanju je učitana",
        booking: updatedBooking,
      });

      res.json({
        message: "Potvrda o plaćanju je uspješno učitana",
  ...
  try {
    ...
    if (userId) {
      io.to(`user_${userId}`).emit("booking_status_updated", {
        message: `Status rezervacije je ažuriran na ${status}`,
        booking: updatedBooking,
      });
    }
